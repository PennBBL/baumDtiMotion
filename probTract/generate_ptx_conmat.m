...
for R=1:X
	...
	% Streamline count %
	%%%%%%%%%%%%%%%%%%%%
	vector_sc=sum(matrix_seeds_to_all_targets);
	...
	length_vec= length_vec ./ sum(matrix_seeds_to_all_targets);
...
% A_raw_sc =(A_raw_sc - diag(diag(A_raw_sc)));
% A_prop =(A_prop - diag(diag(A_prop)));
% A_raw_length =(A_raw_length - diag(diag(A_raw_length)));
...
save(savename,'A_raw_sc', 'A_sc_und', 'A_prop','A_prop_und', 'A_raw_length', 'A_length_und', 'A_volNorm_sc_und', 'volMat')
